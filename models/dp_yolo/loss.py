...
def bbox_iou_w3f(
    ...
    L_f_mpdiou = L_mpdiou + iou - iou.pow(focaler_gamma)
    ...
    R_wiou       = torch.exp(center_dist / gt_diag2)
...
def patch_loss() -> bool:
    ...
    try:
        ...
        _original_bbox_iou = _metrics.bbox_iou  # lưu hàm gốc
        ...
        def _bbox_iou_w3f_compat(
            ...
            """
            Wrapper tương thích: dùng W3F_MPDIoU khi CIoU=True,
            dùng hàm gốc cho các trường hợp còn lại.
            """
            if CIoU:
                return bbox_iou_w3f(box1, box2, xywh=xywh, eps=eps)
            return _original_bbox_iou(
                box1, box2, xywh=xywh,
                GIoU=GIoU, DIoU=DIoU, CIoU=CIoU, eps=eps,
            )

        # Patch cả module-level reference lẫn reference trong utils.loss
        # (quan trọng: utils.loss có thể đã import bbox_iou cục bộ)
        _metrics.bbox_iou  = _bbox_iou_w3f_compat
