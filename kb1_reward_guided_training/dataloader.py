...
def get_train_transforms(img_size: int = 640) -> A.Compose:
    ...
    return A.Compose([
        ...
        A.GaussNoise(std_range=(0.02, 0.12), p=0.3),       # nhieu cam bien UAV
        ...
        A.CLAHE(clip_limit=2.0, p=0.2),                    # tang tuong phan vung toi
        # Native YOLO detect models expect RGB float tensors in [0, 1].
...
def get_val_transforms(img_size: int = 640) -> A.Compose:
    ...
    return A.Compose([
        A.LongestMaxSize(max_size=img_size),
        A.PadIfNeeded(img_size, img_size, border_mode=cv2.BORDER_CONSTANT, fill=114),
        # Keep validation input identical to native YOLO preprocessing.
        A.Normalize(mean=(0.0, 0.0, 0.0),
                    std=(1.0, 1.0, 1.0), max_pixel_value=255.0),
        ToTensorV2(),
    ], bbox_params=A.BboxParams(
