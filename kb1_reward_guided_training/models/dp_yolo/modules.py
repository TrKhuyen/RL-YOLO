...
class TransformerLayer(nn.Module):
    ...
    """
    def __init__(self, c: int, num_heads: int = 4, max_tokens: int = 400):
        super().__init__()
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if H * W > self.max_tokens:
            scale = (self.max_tokens / float(H * W)) ** 0.5
            pooled_h = max(1, min(H, int(H * scale)))
            pooled_w = max(1, min(W, self.max_tokens // pooled_h))
            pooled = F.adaptive_avg_pool2d(x, (pooled_h, pooled_w))

        ph, pw = pooled.shape[-2:]
        seq = pooled.flatten(2).permute(0, 2, 1)
        attn_out, _ = self.attn(seq, seq, seq, need_weights=False)
        seq = self.norm1(seq + attn_out)
        seq = self.norm2(seq + self.ffn(seq))
        out = seq.permute(0, 2, 1).reshape(B, C, ph, pw)
        if (ph, pw) != (H, W):
